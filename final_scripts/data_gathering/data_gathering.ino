/*
Gathers data from various weather senors to send to a VM via mqtt
...
void sendMqttData(char topic[], String data) {
  // send data over mqtt to specified topic
...
unsigned long handleComboData(unsigned long last_sample) {
  // every minute oudoors, every 5 seconds indoors
  if (last_sample + 60000 < millis()) { 
...
unsigned long handleBMEData(unsigned long last_sample) {
  ...
  if (last_sample + 60000 < millis()) {
    ...
    if (adaBME.performReading()) {
      ...
      // send data to mqtt server
      // subtract 3 degrees C from temp reading as the sensor reading is 
      //  quite consistently about 3 degrees higher than the actual temp
      // when sensor is indoors, this difference is closer to 4 degrees
      sendMqttData("weather/bme680/temperatureC", String(adaBME.temperature - 3.0));
      float degrees_f = ((adaBME.temperature - 3.0) * (9.0 / 5.0)) + 32.0;
...
unsigned long handleLightData(unsigned long last_sample) {
  // every 15 seconds outdoors, every second indoors
  if (last_sample + 15000 < millis()) {
    bool valid_light;
...
unsigned long handleLightningData(unsigned long last_sample) {
  // check if lightning was detected every 250 milliseconds whether outdoors or indoors
